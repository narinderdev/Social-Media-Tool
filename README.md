...
- LinkedIn needs an access token and author URN. After setting `LINKEDIN_CLIENT_ID`, `LINKEDIN_CLIENT_SECRET`, and `LINKEDIN_REDIRECT_URI`, open `/api/linkedin/connect` to start the OAuth flow.
...
- Opening a post stats modal allows a manual refresh; background refresh still runs every 3 hours.
